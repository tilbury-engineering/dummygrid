...
function NewsDetail({id}){const live=useLiveNews();const n=live.items.find(x=>String(x.id)===id)||demoNews.find(x=>String(x.id)===id);if(!n)return <NotFound/>;const region=n.country&&n.country!=="Global"?n.country:(n.continent||n.region||"Global");const summary=n.summary||n.dek||"";return <section className="article"><Pill>{region}</Pill><h1>{n.title}</h1><p className="lead">{summary}</p><div className="article-meta">Source: <b>{n.source||"Original publisher"}</b> · {n.published?fmtDate(n.published):(n.time||"")}</div>{n.image&&<div className="article-hero" style={{backgroundImage:`url("${n.image}")`,backgroundSize:"cover",backgroundPosition:"center"}}></div>}<div className="panel article-summary"><h3>DummyGrid summary</h3><p>{summary}</p><p className="muted">This page summarises the published report for the karting community. Copyright in the original reporting remains with its publisher.</p>{n.url&&<a className="button primary" href={n.url} target="_blank" rel="noopener noreferrer">Read original at {n.source||"source"} ↗</a>}</div>{n.sources?.length>0&&<div className="panel"><h3>Additional coverage</h3>{n.sources.map((x,i)=><p key={i}><a className="link" href={x.url} target="_blank" rel="noopener noreferrer">{x.source} ↗</a></p>)}</div>}<Ad format="In-article MPU"/></section>}
...
function AsnsPage(){
 ...
 return <section><PageTitle kicker="National sporting authorities" title="GLOBAL ASN DIRECTORY" text={loading?"Loading authorities…":(data.asns||[]).length+" motorsport authorities indexed. Each country page connects its governing body to karting, circuits, championships and community contributions."}/><Filters><input placeholder="Search authority or country…" value={q} onChange={e=>setQ(e.target.value)}/><select value={continent} onChange={e=>setContinent(e.target.value)}><option>All</option>{continents.map(x=><option key={x}>{x}</option>)}</select></Filters><div className="track-grid">{rows.map(a=><a className="track-card" href={"#/asns/"+a.id} key={a.id}><div className="track-pin">⚑</div><div className="meta">{a.continent} · {a.country}</div><h3>{a.name}</h3><p>{a.abbreviation||"FIA sporting authority"}</p><div className="track-actions"><b className="link">View authority →</b><small>{a.type||"Sport"}</small></div></a>)}</div></section>
}
function AsnDetail({id}){
 ...
 return <section><PageTitle kicker={a.country} title={a.name.toUpperCase()} text="National motorsport authority profile and karting directory." action={<a className="button" href="#/asns">All ASNs</a>}/><div className="track-detail-grid"><div className="panel"><h3>Authority profile</h3><dl className="facts"><div><dt>Country</dt><dd>{a.country}</dd></div><div><dt>Continent</dt><dd>{a.continent}</dd></div><div><dt>FIA membership</dt><dd>{a.type||"Sport"}</dd></div><div><dt>Address</dt><dd>{a.address||"—"}</dd></div></dl>{a.sourceUrl&&<a className="button primary" href={a.sourceUrl} target="_blank" rel="noopener noreferrer">FIA source ↗</a>}</div><div className="panel ugc-callout"><span>COMMUNITY POWERED</span><h3>Help complete this ASN</h3><p>Add karting championships, licence guidance, clubs, officials, documents, photos or corrections. Contributions will be attributed and reviewed.</p><a className="button primary" href={"#/contribute/asn/"+a.id}>Contribute information</a></div></div><SectionHead eyebrow="Karting network" title={"Tracks in "+a.country} copy={tracks.length+" circuits currently connected to this national directory."}/><div className="track-grid">{tracks.slice(0,24).map(t=><a className="track-card" href={"#/tracks/"+t.id} key={t.id}><div className="meta">{t.region||t.country}</div><h3>{t.name}</h3><p>{t.city}</p></a>)}</div></section>
}
function ContributePage({kind,id}){
 return <section><PageTitle kicker="Community contribution" title="HELP BUILD DUMMYGRID" text="Local knowledge is how we build the deepest karting database in the world."/><div className="panel ugc-callout"><span>FREE UGC</span><h3>Contribution system foundation</h3><p>Photos, corrections, facilities, classes, championships, results links, historical notes and ownership claims will attach directly to this {kind} profile.</p><p className="muted">Entity: {id}. Account-backed submissions and moderation are the next backend layer.</p><a className="button" href={kind==="track"?"#/tracks/"+id:"#/asns/"+id}>Back to profile</a></div></section>
}
function TrackDetail({id}){
 ...
 <div className="track-gallery">{(t.images||[]).length?(t.images||[]).slice(0,5).map((img,i)=><figure className={i===0?"hero-photo":""} key={img.url||i}><img src={img.url} alt={img.alt||t.name} loading="lazy"/><figcaption>{img.credit||"Track image"}{img.license?" · "+img.license:""}</figcaption></figure>):<div className="track-photo-placeholder"><b>Photos wanted</b><span>Help document {t.name}</span></div>}</div><div className="track-detail-grid"><div className="panel"><h3>Track profile</h3><dl className="facts"><div><dt>Country</dt><dd>{t.country}</dd></div><div><dt>Region</dt><dd>{t.region||"—"}</dd></div><div><dt>City</dt><dd>{t.city||"—"}</dd></div><div><dt>Address</dt><dd>{t.address||"Address verification pending"}</dd></div><div><dt>What3Words</dt><dd>{t.what3words?<a href={"https://what3words.com/"+t.what3words} target="_blank" rel="noopener noreferrer">///{t.what3words}</a>:<span className="muted">Not published yet</span>}</dd></div><div><dt>Continent</dt><dd>{t.continent||"—"}</dd></div>{t.motorsportUkLicensed&&<div><dt>Motorsport UK</dt><dd>Licensed kart venue</dd></div>}<div><dt>Type</dt><dd>{t.venueType||"—"}</dd></div><div><dt>Karts</dt><dd>{t.kartType||"—"}</dd></div><div><dt>Track length</dt><dd>{t.trackLengthM?t.trackLengthM+" m":"—"}</dd></div>{t.fiaHomologation&&<><div><dt>FIA licence</dt><dd>{t.fiaHomologation.licence}{t.fiaHomologation.grade?" · Grade "+t.fiaHomologation.grade:""}</dd></div><div><dt>FIA valid until</dt><dd>{t.fiaHomologation.validUntil}</dd></div></>}<div><dt>Championships</dt><dd>{(t.series||[]).join(", ")||"—"}</dd></div></dl>{t.website&&<a className="button primary" href={t.website} target="_blank" rel="noopener noreferrer">Official track website ↗</a>}</div><div className="panel"><h3>DummyGrid archive</h3><p>{linked.length?"We currently link "+linked.length+" result record"+(linked.length===1?"":"s")+" to this circuit.":"No indexed results for this circuit yet."}</p><p className="muted">{t.source}</p></div><div className="panel ugc-callout"><span>COMMUNITY POWERED</span><h3>Know this track?</h3><p>Add photos, facilities, layouts, classes, race series, local tips, corrections or historical information.</p><a className="button primary" href={"#/contribute/track/"+t.id}>Add to this track</a></div></div>
...
function App(){
 ...
 return <><Top region={region} setRegion={setRegion}/><main>{page}</main><footer className="site-footer"><div className="footer-grid"><div className="footer-column"><h3>DummyGrid</h3><a href="#/">Home</a><a href="#/about">About Us</a></div><div className="footer-column"><h3>Main Menu</h3><a href="#/">Home</a><a href="#/news">News</a><a href="#/marketplace">Marketplace</a><a href="#/drivers">Drivers</a><a href="#/classes">Classes</a><a href="#/manufacturers">Manufacturers</a><a href="#/asns">ASNs</a><a href="#/results">Results</a><a href="#/tracks">Tracks</a><a href="#/community">Community</a><a href="#/knowledge-base">Knowledge Base</a></div><div className="footer-column"><h3>Knowledge Base</h3><a href="#/knowledge-base">Karting Knowledge Base</a><a href="#/classes">Karting Classes</a><a href="#/tracks">Track Directory</a><a href="#/asns">National Authorities</a><a href="#/results">Results Archive</a></div><div className="footer-column"><h3>Connect with us</h3><a href="#/community">Community</a><a href="#/profile">Driver Hub</a><a href="#/advertise">Advertise</a><a href="#/contact">Contact Us</a></div></div><div className="footer-bottom"><a className="logo footer-logo" href="#/" aria-label="DummyGrid home"><img src={DUMMYGRID_LOGO} alt="DummyGrid"/></a><p>The world of karting, local to you.</p></div></footer></>
}
