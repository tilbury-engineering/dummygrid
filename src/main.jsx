...
function TrackDetail({id}){
 ...
 <div className="track-detail-grid"><div className="panel"><h3>Track profile</h3><dl className="facts"><div><dt>Country</dt><dd>{t.country}</dd></div><div><dt>Region</dt><dd>{t.region||"—"}</dd></div><div><dt>City</dt><dd>{t.city||"—"}</dd></div><div><dt>Address</dt><dd>{t.address||"Address verification pending"}</dd></div><div><dt>What3Words</dt><dd>{t.what3words?<a href={"https://what3words.com/"+t.what3words} target="_blank" rel="noopener noreferrer">///{t.what3words}</a>:<span className="muted">Not published yet</span>}</dd></div><div><dt>Continent</dt><dd>{t.continent||"—"}</dd></div>{t.motorsportUkLicensed&&<div><dt>Motorsport UK</dt><dd>Licensed kart venue</dd></div>}<div><dt>Type</dt><dd>{t.venueType||"—"}</dd></div><div><dt>Karts</dt><dd>{t.kartType||"—"}</dd></div><div><dt>Track length</dt><dd>{t.trackLengthM?t.trackLengthM+" m":"—"}</dd></div>{t.fiaHomologation&&<><div><dt>FIA licence</dt><dd>{t.fiaHomologation.licence}{t.fiaHomologation.grade?" · Grade "+t.fiaHomologation.grade:""}</dd></div><div><dt>FIA valid until</dt><dd>{t.fiaHomologation.validUntil}</dd></div></>}<div><dt>Championships</dt><dd>{(t.series||[]).join(", ")||"—"}</dd></div>{t.motorsportUkLicensed&&<div><dt>Motorsport UK</dt><dd>Licensed kart venue</dd></div>}</dl>{t.website&&<a className="button primary" href={t.website} target="_blank" rel="noopener noreferrer">Official track website ↗</a>}</div><div className="panel"><h3>DummyGrid archive</h3><p>{linked.length?"We currently link "+linked.length+" result record"+(linked.length===1?"":"s")+" to this circuit.":"No indexed results for this circuit yet."}</p><p className="muted">{t.source}</p></div></div>
