...
function HallOfFameProfile({name}){
 ...
 return <section>
  ...
  <div className="driver-profile-hero">
   ...
   <div className="driver-profile-summary"><div className="meta">HALL OF FAME ENTRY · {driver.year}</div><h2>{driver.name}</h2><p className="driver-profile-country">{special?.nationality||driver.country}{special?.birth?" · "+special.birth:""}{special?.death?" – "+special.death:""}</p>{wiki?.description&&<p className="driver-wiki-role">{wiki.description}</p>}<div className="driver-badges">{disciplines.map(x=><span key={x}>{x}</span>)}</div></div>
  </div>
