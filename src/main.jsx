...
const manufacturers = [
 {id:"tony-kart",name:"Tony Kart",group:"OTK Kart Group",country:"Italy",founded:"1958",site:"https://www.tonykart.com/",aliases:["Tony Kart","OTK"],history:"Founded in 1958, Tony Kart became one of karting's best-known Italian chassis marques and today sits within OTK Kart Group.",products:["Racer 401","Rookie","Mini","KZ / gearbox chassis"],newsTerms:["tony kart","otk"]},
 ...
 {id:"zip-kart",name:"Zip Kart",group:"Zip Kart International",country:"United Kingdom",founded:"1960s",site:"https://zipkart.com/",aliases:["Zip Kart","ZIP"],history:"Zip Kart is one of Britain's long-established karting marques, with roots stretching back to the 1960s and a long record in British karting, engineering and driver development.",products:["Edge 950","Edge","Cadet chassis","Historic Vanquish range"],newsTerms:["zip kart","zipkart"]},
 ...
 {id:"gillard",name:"Gillard Kart",group:"OTK Kart Group",country:"United Kingdom / Italy",founded:"1980",site:"https://www.gillardkart.com/",aliases:["Gillard","Gillard Kart"],history:"Founded in Britain by Tim Gillard in 1980, Gillard became one of the UK's most successful chassis marques. OTK Kart Group acquired the brand in 2020 and continues its product development and production.",products:["TG17","TDX","Rookie EVM / EVS","Direct Drive / Shifter / DD2 chassis"],newsTerms:["gillard kart","gillard"]},
...
function Manufacturers(){
 ...
 return <section><PageTitle kicker="Brands & builders" title="KART MANUFACTURERS" text="Explore chassis manufacturers, their history, product families and the latest news around each brand."/><Filters><input placeholder="Search manufacturer, country or product…" value={q} onChange={e=>setQ(e.target.value)}/></Filters><div className="manufacturer-grid">{rows.map(m=><a className="manufacturer-card" href={"#/manufacturers/"+m.id} key={m.id}><div className="manufacturer-badge">{m.name.slice(0,2).toUpperCase()}</div><div className="meta">{m.country} · {m.group}</div><h3>{m.name}</h3><p>{m.history}</p><div className="product-tags">{m.products.slice(0,3).map(p=><span key={p}>{p}</span>)}</div><b className="link">View manufacturer →</b></a>)}</div></section>
}
function ManufacturerDetail({id}){
 ...
 const related=live.items.filter(n=>terms.some(t=>(n.title+" "+(n.summary||"")+" "+(n.source||"")).toLowerCase().includes(t))).slice(0,9);
 return <section><div className="manufacturer-hero"><div><div className="meta">{m.country} · {m.group} · Founded {m.founded}</div><h1>{m.name}</h1><p>{m.history}</p><div className="actions"><a className="button primary" href={m.site} target="_blank" rel="noopener noreferrer">Official website ↗</a><a className="button" href="#/manufacturers">All manufacturers</a></div></div><div className="manufacturer-mark">{m.name.slice(0,2).toUpperCase()}</div></div><div className="two-col"><div className="panel"><h3>Product range</h3><div className="product-list">{m.products.map(p=><div key={p}><b>{p}</b><span>Current / representative product family</span></div>)}</div></div><div className="panel"><h3>Brand snapshot</h3><p><b>Manufacturer:</b> {m.name}</p><p><b>Group:</b> {m.group}</p><p><b>Country:</b> {m.country}</p><p><b>Founded:</b> {m.founded}</p></div></div><SectionHead eyebrow="Latest coverage" title={m.name+" news"} copy="Live stories from the KARTGRID news index that mention this manufacturer."/><div className="news-grid">{related.length?related.map((n,i)=><NewsCard key={n.id||i} n={n} big={i===0}/>):<Empty text={"No current "+m.name+" stories in the live feed yet."}/>}</div><Ad format="Manufacturer sponsor"/></section>
}
