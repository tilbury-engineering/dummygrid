...
function getCountryFlag(country){const flags={"United Kingdom":"🇬🇧","USA":"🇺🇸","United States":"🇺🇸","Brazil":"🇧🇷","France":"🇫🇷","Germany":"🇩🇪","Italy":"🇮🇹","Spain":"🇪🇸","Finland":"🇫🇮","Netherlands":"🇳🇱","Monaco":"🇲🇨","Australia":"🇦🇺","Canada":"🇨🇦","Colombia":"🇨🇴","New Zealand":"🇳🇿","Belgium":"🇧🇪","Switzerland":"🇨🇭","Sweden":"🇸🇪","Austria":"🇦🇹","Japan":"🇯🇵","Poland":"🇵🇱","Denmark":"🇩🇰","Venezuela":"🇻🇪","Mexico":"🇲🇽"};return flags[country]||"🏁"}
function HallOfFameProfile({name}){
 ...
 const disciplines=["Formula 1","IndyCar","Le Mans","Touring Cars","DTM","Formula E","WEC"].filter(x=>driver.path.includes(x)); const special=specialProfiles[driver.name];
 ...
 return <section>
  ...
  <div className="driver-profile-hero">
   ...
   <div className="driver-profile-summary"><div className="meta">HALL OF FAME ENTRY · {driver.year}</div><h2>{driver.name}</h2><div className="driver-profile-country-row"><p className="driver-profile-country">{special?.nationality||driver.country}{special?.birth?" · "+special.birth:""}{special?.death?" – "+special.death:""}</p><span className="driver-profile-flag" aria-label={(special?.nationality||driver.country)+" national flag"}>{getCountryFlag(special?.nationality||driver.country)}</span></div>{wiki?.description&&<p className="driver-wiki-role">{wiki.description}</p>}<div className="driver-badges">{disciplines.map(x=><span key={x}>{x}</span>)}</div></div>
  </div>
  ...
  {special&&<div className="profile-three-col special-profile-grid"><div className="panel"><div className="meta">CAREER & CONTRIBUTION</div><h3>{special.title}</h3><p>{special.career}</p></div><div className="panel"><div className="meta">MAJOR TITLES / ACHIEVEMENTS</div><h3>Record</h3><p>{special.titles}</p></div><div className="panel"><div className="meta">LEGACY</div><h3>Impact on karting</h3><p>{special.legacy}</p><p>{special.proteges}</p></div></div>}<div className="profile-three-col">
  ...
  <div className="panel driver-career"><div className="meta">HALL OF FAME RECORD</div><h3>Championship record & legacy</h3><p>{special?.source||`${driver.name} is recorded here because the driver's career connects an identifiable karting championship achievement with later success at internationally recognised championship level.`} KartGrid will continue to expand this profile with verified championship seasons, titles, race statistics, teams, manufacturers and career milestones.</p>{wiki?.wikibase_item&&<span className="source-tag">Wikidata: {wiki.wikibase_item}</span>}</div>
 </section>
}
