...
function AsnsPage(){
 ...
 return <section><PageTitle kicker="National sporting authorities" title="GLOBAL ASN DIRECTORY" text={loading?"Loading authorities…":(data.asns||[]).length+" motorsport authorities indexed. Each country page connects its governing body to karting, circuits, championships and community contributions."}/><a className="fia-directory-card" href="https://www.fia.com/" target="_blank" rel="noopener noreferrer"><div className="fia-brand"><img className="fia-official-logo" src="https://commons.wikimedia.org/wiki/Special:Redirect/file/FIA_logo_(transparent).png" alt="FIA official logo"/><div><div className="meta">WORLD MOTOR SPORT GOVERNING BODY</div><h2>Fédération Internationale de l’Automobile</h2><p>The global federation for motor sport and the parent organisation of the national sporting authorities listed below.</p></div></div><span className="link">Visit FIA ↗</span></a><Filters><input placeholder="Search authority or country…" value={q} onChange={e=>setQ(e.target.value)}/><select value={continent} onChange={e=>setContinent(e.target.value)}><option>All</option>{continents.map(x=><option key={x}>{x}</option>)}</select></Filters><div className="asn-grid">{rows.map(a=><a className="asn-card" href={"#/asns/"+a.id} key={a.id}><div className="asn-card-head"><span className="asn-flag"><img src={a.flagUrl} alt={a.country+" national flag"} loading="lazy"/></span>{a.logoUrl?<img className="asn-logo" src={a.logoUrl} alt={a.name+" logo"} loading="lazy" onError={e=>{e.currentTarget.style.display="none"}}/>:<span className="asn-logo-fallback">ASN</span>}</div><div className="meta">{a.continent} · {a.country}</div><h3>{a.name}</h3><p>{a.abbreviation||"FIA sporting authority"}</p><div className="track-actions"><b className="link">View authority →</b><small>{a.type||"Sport"}</small></div></a>)}</div></section>
}
