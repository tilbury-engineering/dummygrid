...
function AsnsPage(){
 ...
 return <section><PageTitle kicker="National sporting authorities" title="GLOBAL ASN DIRECTORY" text={loading?"Loading authorities…":(data.asns||[]).length+" motorsport authorities indexed. Each country page connects its governing body to karting, circuits, championships and community contributions."}/><Filters><input placeholder="Search authority or country…" value={q} onChange={e=>setQ(e.target.value)}/><select value={continent} onChange={e=>setContinent(e.target.value)}><option>All</option>{continents.map(x=><option key={x}>{x}</option>)}</select></Filters><div className="asn-grid">{rows.map(a=><a className="asn-card" href={"#/asns/"+a.id} key={a.id}><div className="asn-card-head"><span className="asn-flag" aria-label={a.country+" flag"}>{a.flag||"🏁"}</span>{a.logoUrl?<img className="asn-logo" src={a.logoUrl} alt={a.name+" logo"} loading="lazy" onError={e=>{e.currentTarget.style.display="none"}}/>:<span className="asn-logo-fallback">ASN</span>}</div><div className="meta">{a.continent} · {a.country}</div><h3>{a.name}</h3><p>{a.abbreviation||"FIA sporting authority"}</p><div className="track-actions"><b className="link">View authority →</b><small>{a.type||"Sport"}</small></div></a>)}</div></section>
}
