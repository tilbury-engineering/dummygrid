...
function HallOfFame(){
 ...
 <div className="hof-grid">{rows.sort((a,b)=>a.year-b.year||a.name.localeCompare(b.name)).map((d,i)=><a className="hof-card" key={d.name} href={"#/hall-of-fame/driver/"+encodeURIComponent(d.name)}><DriverPortrait name={d.name}/><div className="hof-year"><span>QUALIFIED</span><strong>{d.year}</strong></div><div className="hof-card-meta"><div className="hof-country">{d.country}</div><div className="hof-qualification">{qualificationLabel(d)}</div></div><h3>{d.name}</h3><div className="hof-path">{d.path}</div><p>{d.note}</p><div className="hof-footer"><span>Karting → {d.path.split("→").slice(-1)[0].trim()}</span><b>View profile →</b></div></a>)}</div>
